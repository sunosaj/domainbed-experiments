...
class DADR(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.aug_optimizer = torch.optim.Adam(
            self.cvae.parameters(),
            lr=self.hparams["lr"],
            weight_decay=self.hparams['weight_decay']
        )

    def loss_fn_aug(self, recon_x, x, mean, log_var):
    ...
    def update_augmentor(self, minibatches, unlabeled=None):
        ...
        recon_x, means, log_var, d, yhat = self.cvae(all_x)
        loss = self.loss_fn_aug(recon_x, all_x, means, log_var) + F.cross_entropy(yhat, all_y) + F.mse_loss(all_c_d, d)
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        all_y = torch.cat([y for x, y, c_d in minibatches])
        # all_c_d = torch.cat([c_d for x,y,c_d in minibatches])
        aug_x, aug_y = self.cvae(mode='dom_aug') # import matplotlib.pyplot as plt;plt.imshow(aug_x[0].cpu().detach().permute(1, 2, 0));plt.show()
        all_x = torch.cat([all_x, aug_x])
        all_y = torch.cat([all_y, aug_y])
        loss = F.cross_entropy(self.predict(all_x), all_y)
